...
class OcrToTableTool:
    ...
    def __init__(self, image, original_image, table_name):
    ...
    def draw_contours(self):
        ...
        cv2.drawContours(self.image_with_contours, self.approximated_contours, -1, (0, 255, 0), 3)
    ...
    def convert_contours_to_bounding_boxes(self):
        ...
        for contour in self.contours:
            ...
            self.bounding_boxes.append((x, y-2, w, h+2))
            y = y -4
            h = h + 3
    ...
    def remove_leading_commas(self, text):
      lines = text.splitlines()
      cleaned_lines = [re.sub(r"^,", "", line) for line in lines]
      return "\n".join(cleaned_lines)
    
    def generate_csv_file(self):
        with open(self.table_name+"_output.csv", "w") as f:
            for row in self.table:
                row = ",".join(row)
                f.write(self.remove_leading_commas(row) + "\n")
